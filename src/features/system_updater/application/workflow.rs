...
impl StepSelection {
    ...
    where
        ...
        R: RunReporter,
    {
        if self.exclude_groups.contains(&step.group) || self.exclude_steps.contains(step.id) {
            return false;
        }
        ...
        if let Some(include_steps) = &self.include_steps
            && !include_steps.contains(step.id)
        {
            return false;
        }
        true
...
#[cfg(test)]
mod tests {
    use super::{StepDefinition, StepSelection, run};
    use crate::features::system_updater::domain::config::Config;
    use crate::features::system_updater::domain::platform::PlatformInfo;
    ...
    fn workflow_marks_gb10_steps_as_skipped_on_generic_linux() {
        ...
        let reporter = FakeReporter::new();
        let config = Config::default();
        let platform = PlatformInfo::generic_linux(Some("Generic VM".into()));
